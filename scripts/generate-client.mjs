...
const typesSource = `// Generated from contracts/openapi/openapi.json. Do not edit by hand.\n\n${Object.entries(
  ...
  .map(
    (operation) =>
      `  ${propertyName(operation.id)}: { method: ${JSON.stringify(operation.method)}; path: ${JSON.stringify(operation.path)}; pathParams: ${operation.pathType}; body: ${operation.bodyType}; response: ${operation.responseType}; requiresIdempotency: ${operation.idempotency}; };`,
  )
...
const outputs = new Map([
  ...
  [path.join(outputDir, "client.ts"), clientSource],
