...
if ($Action -eq "reset") {
    docker compose exec -T cockroach cockroach sql --insecure --host=localhost:26257 `
        --execute="DROP DATABASE IF EXISTS sira CASCADE; CREATE DATABASE sira;"
}
else {
    docker compose exec -T cockroach cockroach sql --insecure --host=localhost:26257 `
        --execute="CREATE DATABASE IF NOT EXISTS sira; CREATE USER IF NOT EXISTS sira_app;"
}
...
docker compose exec -T cockroach cockroach sql --insecure --host=localhost:26257 --database=sira `
    --execute="GRANT CONNECT ON DATABASE sira TO sira_app; GRANT sira_runtime TO sira_app;"
if ($LASTEXITCODE -ne 0) {
