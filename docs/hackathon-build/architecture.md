...
| Agent runtime | A small typed cognitive kernel uses Bedrock Converse directly. Separate principal-locked SIRA and SEIL AgentCore Runtime deployments provide the hosted model boundary. Bedrock Agents Classic is excluded. |
...
| Payment | Provider-neutral external handoff is P2 after every P0 gate. Agents never hold payment credentials. |
| Multi-region/AgentCore/changefeeds | Two stateless AgentCore runtimes isolate SIRA and SEIL. They receive signed, scoped manifests and no database credentials; validated results are committed through CockroachDB. AgentCore Memory remains excluded. Changefeed ingress writes only to an isolated hint queue; a separate worker re-reads CockroachDB, invalidates stale current decisions and schedules one replacement through the ordinary outbox. Multi-region requires measured need. |
...
Implemented P2 includes tenant-safe buyer/seller operational analytics derived from canonical records and transactional outbox events, explicitly labelled observational rather than causal. It also includes a replayable experiment contract, durable budgeted lifecycle and separate stateless ARM64 AgentCore runtimes for SIRA and SEIL. Network/model work occurs outside Cockroach transactions; typed, hashed observations or sanitized failure categories are persisted afterward. AgentCore has no database credential or Memory resource and cannot authorize commercial effects. A versioned Bedrock Automated Reasoning policy reviews bilateral-consent and purchase-authority claims as explanatory evidence only. An authenticated Lambda bridge converts at-least-once Cockroach changefeed deliveries into deterministic hints on an isolated FIFO queue; a separate worker re-reads active pointers, invalidates affected current decisions and emits one ordinary replacement event. Remaining P2 evidence is live AgentCore, Automated Reasoning and changefeed execution, measured multi-region behavior, more categories and autonomous commercial operations only where authority and provider idempotency are proven. Payment processing is intentionally excluded; the product ends at a provider-neutral human-approved handoff.
