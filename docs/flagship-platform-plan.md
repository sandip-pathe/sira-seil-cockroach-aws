...
| R3 hardening | Partial | Static quality, credential scan, non-provider tests, web checks, architecture boundaries, browser smoke, a sanitized local API latency gate, and a 50-turn labelled conversation corpus/harness pass structurally. Hosted provider execution, load, chaos, managed restore, five hosted rehearsals, and provider cost/latency remain evidence gates. |
